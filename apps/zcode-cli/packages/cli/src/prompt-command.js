...
import { buildManualSkillPrompt, createCommandCenter, formatSlashCommandHelp, parseSlashCommand, } from "./command-center.js";
import { loadCliDotenv } from "./env.js";
import { createCliHeadlessBrowserRuntime } from "./headless-browser.js";
import { createHeadlessPermissionBroker, createHeadlessSessionObserver, readHeadlessRuntimeFacts, waitForHeadlessWorkflowSettle, } from "./headless-workflow.js";
import { resolveResumeSession } from "./resume.js";
import { readRuntimeEventSubscriber } from "./runtime-event-subscriber.js";
import { DEFAULT_CLI_CLEANUP_TIMEOUT_MS, registerCliShutdownHandlers, runCliCleanupWithTimeout, } from "./shutdown.js";
 ...
 */
const wantsJsonSummary = (options) => options.outputFormat === undefined
    ? options.json
...
const MEMORY_BENCH_DISABLED_ERROR = "--memory-bench requires Project Memory to be enabled (features.memory=true and memory.use=true).";
...
export const runPrompt = async (ctx, prompt, attachmentPaths, options, deps, version, mode, resumeRequest = { continueSession: false }, toolDisallowlist, forceMcs = false, presentationSurface = "terminal") => {
    ...
    if (slashCommand?.type === "known" && slashCommand.name === "help") {
        ctx.stdout.write(`${formatSlashCommandHelp(slashCommand.args, await listCustomCommandsForPrompt(deps))}\n`);
        return 0;
    }
    if (slashCommand?.type === "known" && slashCommand.name === "skill" && !slashCommand.skillName) {
        return await runSkillsCommand(ctx, options, deps, []);
    }
    const runtimePrompt = slashCommand?.type === "known" && slashCommand.name === "skill"
    ...
    try {
        ...
        if (!createApp)
            throw new Error("ZCode app factory is unavailable.");
        ...
        const appEnv = env;
        const startProviderRegistryRuntime = deps.startProcessProviderRegistryRuntime ??
        ...
        providerRegistryRuntime = await startProviderRegistryRuntime(appEnv, deps.skipUserConfig
            ? {}
            : {
                standalone: {
        ...
        if (slashCommand && (await routesToPromptCommandCenter(slashCommand, deps))) {
            return await runPromptCommandCenterCommand(ctx, options, app, prompt, mode, traceId, abortController.signal, deps);
        }
        ...
        const result = await app.submitPrompt(attachmentPaths.length > 0
            ? {
                text: runtimePrompt,
        ...
        const turnResponses = [result.response, ...observer.waitPhaseTurnResponses()].filter((text) => text.trim().length > 0);
        ...
        if (hookTrustDiagnostic)
            writeHeadlessWorkspaceHookTrustDiagnostic(ctx, hookTrustDiagnostic);
...
async function routesToPromptCommandCenter(slashCommand, deps) {
...
async function isResolvableCustomCommand(deps, name) {
    ...
    if (await isReservedSlashCommandName(deps, name))
        return false;
    try {
...
async function isReservedSlashCommandName(deps, name) {
    if (deps.isReservedSlashCommandName)
        return deps.isReservedSlashCommandName(name);
...
async function listCustomCommandsForPrompt(deps) {
    const env = deps.env ?? process.env;
    const workingDirectory = (deps.cwd ?? process.cwd)();
...
async function loadCustomCommandForPrompt(deps, name) {
    const env = deps.env ?? process.env;
    const workingDirectory = (deps.cwd ?? process.cwd)();
...
async function resolveHeadlessWorkspaceHookTrustDiagnostic(input) {
    ...
    if (!reasonCode)
        return undefined;
    const inspect = input.deps.inspectWorkspaceHookTrust ?? input.bootstrapModule?.inspectWorkspaceHookTrust;
    if (!inspect)
        return undefined;
...
function isHeadlessWorkspaceHookBlockReason(value) {
    return HEADLESS_WORKSPACE_HOOK_BLOCK_REASONS.some((candidate) => candidate === value);
}
function writeHeadlessWorkspaceHookTrustDiagnostic(ctx, status) {
    ctx.stderr.write([
