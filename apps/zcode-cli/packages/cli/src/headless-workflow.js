import { AMEND_WORKFLOW_TOOL_NAME, CREATE_WORKFLOW_TOOL_NAME, SessionEventType, } from "@zcode/contracts";
 ...
 */
export const createHeadlessPermissionBroker = () => {
    ...
    return {
        requestPermission: async (request, options) => {
            ...
            if (request.toolName !== CREATE_WORKFLOW_TOOL_NAME &&
...
const isDynamicWorkflowRunProgressEvent = (event) => event.type === SessionEventType.DynamicWorkflowRunProgress;
const progressPayloadOf = (event) => event.payload;
/**
 ...
 */
const mapWorkflowRunProgressStreamLine = (event) => ({
...
const describeProgress = (payload) => {
    ...
    switch (payload.eventType) {
        ...
        case "run-settled": {
            ...
            const status = typeof inner.stopReason === "string" ? `${rawStatus}/${inner.stopReason}` : rawStatus;
 ...
 * 而作者写下它就是为了被看到。
 */
const isUnthrottledProgress = (eventType) => eventType === "run-started" || eventType === "run-settled" || eventType === "log";
 ...
 */
const createWorkflowProgressReporter = (input) => {
    const now = input.now ?? Date.now;
    const throttleMs = input.throttleMs ?? DEFAULT_WORKFLOW_PROGRESS_THROTTLE_MS;
    let lastThrottledAt;
    return (event) => {
        if (!isDynamicWorkflowRunProgressEvent(event))
            return;
        ...
        if (!isUnthrottledProgress(payload.eventType)) {
            const at = now();
            if (lastThrottledAt !== undefined && at - lastThrottledAt < throttleMs)
                return;
            lastThrottledAt = at;
        }
 ...
 */
export const createHeadlessSessionObserver = (input) => {
    ...
    const writeStreamEvent = mapSessionEvent
        ...
        : undefined;
    const reportProgress = mapSessionEvent === undefined && wantsWorkflowProgress(options)
    ...
    return {
        ...
        observe: (event) => {
            ...
            if (isWorkflowActivityEvent(event))
                workflowActivity = true;
            if (!waiting || event.type !== SessionEventType.TurnComplete)
                return;
            // 首个回合的文本来自 submitPrompt 的返回值；它的 turn_complete 若迟到就会重复计数。
            if (event.turnId !== undefined && String(event.turnId) === excludedTurnId)
                return;
 ...
 */
const isWorkflowActivityEvent = (event) => {
    if (isDynamicWorkflowRunProgressEvent(event))
        return true;
    return (event.type === SessionEventType.BackgroundTaskStarted &&
        event.payload.taskKind === "workflow");
};
 ...
 * 而不是假装等过了。
 */
export const readHeadlessRuntimeFacts = (runtime) => {
 ...
 */
const wantsWorkflowProgress = (options) => options.outputFormat === undefined ? !options.json : options.outputFormat === "text";
