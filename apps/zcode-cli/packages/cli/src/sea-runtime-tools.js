...
import { getRuntimeToolRuntime } from "@zcode/shared/runtime-tool-runtime";
...
const runtimeToolIds = new Set(["bfs", "ripgrep", "ugrep"]);
export async function ensureSeaRuntimeTools(options = {}) {
    ...
    if (manifest.target !== target) {
        throw new Error(`SEA runtime tool target mismatch: binary contains ${manifest.target}, runtime is ${target}.`);
    }
    const env = options.env ?? process.env;
    const configuredStorageRoot = options.storageRoot ?? env.ZCODE_STORAGE_DIR?.trim();
    const storageRoot = configuredStorageRoot || join(homedir(), OPENZWORK_DATA_DIR_NAME);
    const runtimeEnv = {};
    for (const tool of manifest.tools) {
        const runtime = getRuntimeToolRuntime(tool.id);
        if (env[runtime.binaryEnvVar]?.trim())
            continue;
...
async function ensureRuntimeTool(input) {
    ...
    const cacheDirectory = join(storageRoot, "cache", "runtime_tools", manifest.target, tool.id, `${tool.version}-${tool.sha256}`);
    const binaryPath = join(cacheDirectory, tool.binaryName);
    const marker = markerFor(manifest, tool);
    if (await isCacheCurrent(cacheDirectory, marker, platform))
        return binaryPath;
...
async function installCacheDirectory(temporaryDirectory, cacheDirectory, marker, platform) {
    for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
            ...
            if (await isCacheCurrent(cacheDirectory, marker, platform))
                return;
            if (!isDirectoryReplaceRace(error) || attempt === 1)
                throw error;
...
function readManifest(sea) {
    let manifest;
    try {
        manifest = JSON.parse(sea.getAsset(manifestAssetKey, "utf8"));
    }
    catch (error) {
...
async function isCacheCurrent(cacheDirectory, expected, platform) {
    try {
        const marker = JSON.parse(await readFile(join(cacheDirectory, markerFileName), "utf8"));
        ...
        if (!binaryStat.isFile() || binaryStat.size !== expected.size)
            return false;
...
function markerFor(manifest, tool) {
    return {
...
function isDirectoryReplaceRace(error) {
