...
export const parseGlobalArgs = (argv) => parseArgs({
...
export function isProtocolServerInvocation(argv) {
    try {
        const parsed = parseGlobalArgs(argv);
        return (parsed.values.prompt === undefined &&
...
export const extractDisallowedToolsArgs = (argv) => {
    const args = [];
    const values = [];
...
const isCliOptionToken = (value) => value === "--" || /^-[^-]?|^--/.test(value);
const normalizeCliToolRuleList = (values) => {
    const normalized = [];
    const seen = new Set();
...
const normalizeCliToolRule = (rule) => {
    if (!rule)
        return "";
    if (rule === "web_search")
        return "WebSearch";
    if (rule.startsWith("web_search("))
        return `WebSearch${rule.slice("web_search".length)}`;
    return rule;
};
