...
export function expandCliCustomCommandPrompt(input) {
    if (usesUnsupportedDynamicShell(input.command.content)) {
        throw new Error(`Custom command /${input.command.metadata.name} uses unsupported shell expansion. Dynamic expansion is not available yet.`);
    }
    ...
    body = body.replace(POSITIONAL_ARGUMENT_PATTERN, (_match, index) => {
...
function formatCommandSkillInstructions(skills) {
    if (skills.length === 0)
        return [];
...
function splitCliCustomCommandArguments(input) {
    const args = [];
    let current = "";
    let escaping = false;
    let quote = null;
...
function usesUnsupportedDynamicShell(content) {
