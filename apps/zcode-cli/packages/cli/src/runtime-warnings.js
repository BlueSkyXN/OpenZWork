const SQLITE_EXPERIMENTAL_WARNING = /^\(node:\d+\) ExperimentalWarning: SQLite is an experimental feature and might change at any time\r?$/;
const MODULE_REGISTER_DEPRECATION_WARNING = /^\(node:\d+\) \[DEP0205\] DeprecationWarning: `module\.register\(\)` is deprecated\. Use `module\.registerHooks\(\)` instead\.\r?$/;
const NODE_TLS_REJECT_UNAUTHORIZED_WARNING = /^\(node:\d+\) Warning: Setting the NODE_TLS_REJECT_UNAUTHORIZED environment variable to '0' makes TLS connections and HTTPS requests insecure by disabling certificate verification\.\r?$/;
const TRACE_WARNINGS_HINT = /^\(Use `[^`]+ --trace-(?:warnings|deprecation) \.\.\.` to show where the warning was created\)\r?$/;
const AI_SDK_WARNING_SYSTEM = /^AI SDK Warning System: To turn off warning logging, set the AI_SDK_LOG_WARNINGS global to false\.\r?$/;
const AI_SDK_ANTHROPIC_THINKING_BUDGET_WARNING = /^AI SDK Warning \(anthropic\.messages \/ [^)]+\): The feature "extended thinking" is used in a compatibility mode\. thinking budget is required when thinking is enabled\. using default budget of \d+ tokens\.\r?$/;
export function interceptKnownRuntimeWarnings(stderr) {
    ...
    stderr.write = ((chunk, encodingOrCallback, callback) => {
        ...
        if (text.length === 0)
            return writeOriginal(chunk, encodingOrCallback, callback);
...
function filterKnownRuntimeWarningChunk(text, state) {
    const segments = text.match(/[^\n]*\n|[^\n]+/g) ?? [];
    let output = "";
    for (const segment of segments) {
        const line = segment.endsWith("\n") ? segment.slice(0, -1) : segment;
        if (SQLITE_EXPERIMENTAL_WARNING.test(line) ||
