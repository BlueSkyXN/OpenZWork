...
export function createWorkspacePathSuggestionProvider(options) {
    ...
    return async (request) => {
        ...
        try {
            const listed = await fileSystemPort.listDirectory({ path: directory }, { signal: request.abortSignal });
            const candidates = listed.entries
                .flatMap((entry) => {
                if (entry.kind !== "directory" && entry.kind !== "file")
                    return [];
                if (entry.kind === "directory" && EXCLUDED_DIRECTORY_NAMES.has(entry.name))
                    return [];
                if (!parsed.query.startsWith(".") && entry.name.startsWith("."))
                    return [];
                if (!matchesPathQuery(entry.name, parsed.query))
                    return [];
            ...
            const limit = Math.max(1, Math.floor(request.limit ?? DEFAULT_WORKSPACE_PATH_SUGGESTION_LIMIT));
            return {
...
function parseWorkspacePathToken(token) {
...
function matchesPathQuery(name, query) {
    if (query.length === 0)
        return true;
    return name.toLowerCase().includes(query.toLowerCase());
}
function compareWorkspacePathSuggestion(left, right, query) {
...
function basenameFromSuggestion(path) {
...
function toWorkspaceRelativePath(workspaceDirectory, path) {
