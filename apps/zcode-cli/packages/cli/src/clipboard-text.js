...
export function createNodeClipboardTextWriter(options = {}) {
...
function writeOsc52(stdout, buffer) {
    if (!stdout)
        return false;
    try {
        stdout.write(`${OSC_52_PREFIX}${OSC_52_CLIPBOARD_TARGET};${buffer.toString("base64")}${OSC_52_SUFFIX}`);
        return true;
    }
    catch {
        return false;
...
function nativeClipboardCommands(platform) {
    if (platform === "darwin") {
...
function assertClipboardTextSize(text, maxBytes) {
...
async function runCommandText(file, args, options) {
