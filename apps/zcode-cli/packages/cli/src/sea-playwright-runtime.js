...
export async function loadCliPlaywrightChromium() {
...
async function ensureSeaPlaywrightRuntime(sea) {
    const manifest = readManifest(sea);
    const cacheDirectory = join(cacheBaseDirectory(), __CLI_VERSION__, manifest.target, `playwright-${manifest.packageVersion}-${manifest.hash}`);
...
function readManifest(sea) {
    const parsed = JSON.parse(sea.getAsset(MANIFEST_ASSET_KEY, "utf8"));
    if (parsed.version !== 1 ||
        !parsed.hash ||
...
async function isCacheCurrent(markerPath, expected) {
