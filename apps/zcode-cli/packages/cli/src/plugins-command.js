...
import { formatAvailablePluginJson, formatDiagnosticJson, formatDiagnosticLines, formatHumanAvailableList, formatHumanPluginList, formatHumanPluginSet, formatHumanPluginUninstall, formatInstalledPluginJson, formatPluginJson, formatPluginSetJson, writeWarnings, } from "./plugins-command-format.js";
import { ambiguousPluginError, baseOptions, confirmUninstall, hasErrors, PluginsUsageError, reportPluginsError, requireOne, resolveDep, resolveLoadedPluginId, resolveScope, splitPluginIdentifier, } from "./plugins-command-shared.js";
...
export async function runPluginsCommand(ctx, options, deps, args, flags = {}) {
...
async function runPluginsListCommand(ctx, options, deps, flags) {
    ...
    if (!flags.available) {
        ctx.stdout.write(options.json ? formatJson(installed) : formatHumanPluginList(outcome, options));
        // 市场刷新失败这类诊断的 pluginId 是市场 id 或为空，归不到任何条目；error 级不能静默丢掉。
        const knownIds = new Set(outcome.plugins.map((plugin) => plugin.id));
        const orphaned = outcome.diagnostics.filter((diagnostic) => diagnostic.severity === "error" && !(diagnostic.pluginId && knownIds.has(diagnostic.pluginId)));
    ...
    ctx.stdout.write(`${formatHumanPluginList(outcome, options)}\n${formatHumanAvailableList(overview.availablePlugins)}`);
    return 0;
}
async function runPluginsInstallCommand(ctx, options, deps, flags, identifier) {
...
async function runPluginsUpdateCommand(ctx, options, deps, flags, identifier) {
    ...
    if (failed) {
        ctx.stderr.write(`Failed to update plugin ${pluginId}\n${formatDiagnosticLines(result.diagnostics)}`);
        return 1;
    }
    const version = current?.version ?? "";
    ctx.stdout.write(version && version === result.previousVersion
...
function reportInstallOutcome(ctx, options, result, pluginId) {
    const failed = hasErrors(result.diagnostics);
    if (options.json) {
        ctx.stdout.write(formatJson({
            pluginId,
    ...
    if (failed) {
        ctx.stderr.write(`Failed to install plugin ${pluginId}\n${formatDiagnosticLines(result.diagnostics)}`);
        return 1;
    }
    for (const plugin of result.installedPlugins) {
        const version = plugin.version ? ` (${plugin.version})` : "";
        ctx.stdout.write(`Installed plugin ${plugin.id}${version} [${plugin.enabled ? "enabled" : "disabled"}]\n`);
    }
    writeWarnings(ctx, result.diagnostics);
    return 0;
}
async function runPluginsSetCommand(ctx, options, deps, flags, plugin, enabled) {
...
async function runPluginsDisableCommand(ctx, options, deps, flags, rest) {
    if (flags.all && rest.length > 0) {
        throw new PluginsUsageError("Cannot use --all with a specific plugin");
    }
    if (!flags.all) {
        if (rest.length === 0) {
            throw new PluginsUsageError("Please specify a plugin name or use --all to disable all plugins");
        }
        return await runPluginsSetCommand(ctx, options, deps, flags, requireOne(rest), false);
    }
    if (flags.scope !== undefined)
        throw new PluginsUsageError("Cannot use --scope with --all");
    const base = baseOptions(deps);
    ...
    if (options.json) {
        ctx.stdout.write(formatJson(entries.map((entry) => ({
...
async function runPluginsUninstallCommand(ctx, options, deps, flags, identifier) {
    resolveScope(flags.scope);
    const target = await resolveLoadedPluginId(deps, identifier);
    // 卸载是破坏性的彻底清除：交互终端下询问确认；非交互(管道/CI)且未带 --force 时拒绝执行，不静默卸载。
    if (!options.force) {
        if (!ctx.stdin.isTTY) {
            ctx.stderr.write(`Refusing to uninstall ${target} without confirmation. Re-run with --force in a non-interactive shell.\n`);
            return 1;
        }
...
async function runPluginsValidateCommand(ctx, options, deps, path) {
    ...
    if (options.json) {
        ctx.stdout.write(formatJson({ path, ok: !failed, diagnostics: diagnostics.map(formatDiagnosticJson) }));
        return failed ? 1 : 0;
    }
    if (failed) {
