...
import { materializeZCodeBuiltinProviderConfig, NodeZCodeBuiltinProviderConfigSource, PERSONAL_PROVIDER_CONFIG_FILE_NAME, ZCODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV, ZCODE_PERSONAL_PROVIDER_CONFIG_FILE_ENV, } from "@zcode/provider-node";
...
export async function prepareCliProviderRuntimeEnv(options) {
    ...
    const personalFilePath = explicitPersonal ?? join(dataBaseDir, OPENZWORK_DATA_DIR_NAME, "v2", PERSONAL_PROVIDER_CONFIG_FILE_NAME);
    const source = new NodeZCodeBuiltinProviderConfigSource({
        bundledFilePath: zcodeBuiltinFilePath,
        watch: false,
    });
    // Modified for the private fork: read the selected local file, never an endpoint cache.
    try {
        await source.read();
    }
    finally {
