...
export const runSkillsCommand = async (ctx, options, deps, args) => {
...
async function runSkillsListCommand(ctx, options, deps) {
    try {
...
async function runSkillsInspectCommand(ctx, options, deps, name) {
    try {
        const env = deps.env ?? process.env;
        const workingDirectory = (deps.cwd ?? process.cwd)();
        const bootstrap = deps.loadBootstrapModule ?? (() => import("@zcode/bootstrap"));
...
function reportSkillsError(ctx, options, error) {
...
function formatHumanSkillList(outcome, options) {
...
function formatHumanSkillInspection(inspection, options) {
    ...
    if (metadata.pluginName)
        lines.push(`pluginName: ${metadata.pluginName}`);
    if (metadata.qualifiedName)
        lines.push(`qualifiedName: ${metadata.qualifiedName}`);
    if (metadata.whenToUse)
        lines.push(`whenToUse: ${metadata.whenToUse}`);
    lines.push(`safeToAutoLoad: ${metadata.safeToAutoLoad ? "yes" : "no"}`);
    lines.push(`size: ${inspection.skill.bytesRead}/${inspection.skill.sizeBytes} bytes${inspection.skill.truncated ? " (truncated)" : ""}`);
    lines.push("", "Content", inspection.skill.content.length > 0 ? inspection.skill.content : "(empty)");
...
const formatSkillJson = (outcome, cwd) => formatJson({
    cwd,
...
const formatSkillInspectionJson = (inspection, cwd) => formatJson({
    cwd,
