...
import { ZCODE_PROCESS_DIAGNOSTIC_PREFIX, ZCODE_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS, ZCODE_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS, ZCODE_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS, } from "@zcode/shared/process-diagnostic";
/**
 ...
 */
export function installCliProcessErrorBoundary(options) {
    const target = options.target ?? process;
    const stderr = options.stderr ?? process.stderr;
    let monitoredException;
    let fatal = false;
    const reportFatal = (kind, origin, reason) => {
        if (fatal)
            return;
        fatal = true;
...
function writeProcessErrorDiagnostic(stderr, kind, origin, reason) {
    try {
        ...
        const diagnostic = {
            ...
            name: (reason instanceof Error ? reason.name || "Error" : "Error").slice(0, ZCODE_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS),
            message: (reason instanceof Error ? reason.message : detail).slice(0, ZCODE_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS),
        ...
        stderr.write(`${ZCODE_PROCESS_DIAGNOSTIC_PREFIX}${JSON.stringify(diagnostic)}\n[zcode] process error kind=${kind} origin=${origin}\n${detail}\n`);
    }
    catch {
        // 诊断输出不能再次击穿进程级异常边界。
    }
