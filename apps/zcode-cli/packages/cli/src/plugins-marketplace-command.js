...
import { formatDiagnosticJson, formatDiagnosticLines, formatHumanMarketplaceList, formatMarketplaceJson, writeWarnings, } from "./plugins-command-format.js";
import { baseOptions, hasErrors, PluginsUsageError, requireOne, resolveDep, resolveScope, } from "./plugins-command-shared.js";
export async function runMarketplaceCommand(ctx, options, deps, flags, args) {
    ...
    switch (action) {
        ...
        default:
            throw new PluginsUsageError(action ? `Unknown marketplace command: ${action}` : "Missing marketplace command");
    }
}
