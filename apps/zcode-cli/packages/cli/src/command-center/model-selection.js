...
export async function rememberCurrentModelSelection(app, deps) {
    if (!deps.saveDefaultModelSelection)
        return "";
    try {
        const ref = app.getCurrentModelOption?.()?.ref;
        const reasoningLevel = app.getThoughtLevel?.();
        if (!ref || !reasoningLevel)
            throw new Error("Current model selection is incomplete.");
