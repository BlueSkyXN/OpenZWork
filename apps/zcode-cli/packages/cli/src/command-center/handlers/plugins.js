...
const PLUGINS_COMMAND_USAGE = "Usage: /plugins [list|enable <plugin>|disable <plugin>|uninstall <plugin> --force]";
const ENABLED_MARK = "✓";
const DISABLED_MARK = "○";
export async function handlePluginsCommand(args, deps) {
...
async function handlePluginsUninstall(tokens, deps, app) {
...
function buildPluginsSelection(outcome, selectedPluginId, locale = "en-US") {
...
function formatPluginsPanelResponse(outcome) {
...
function formatPluginSetResult(result) {
