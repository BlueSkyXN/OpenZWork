export async function handleSkillListCommand(deps) {
    if (!deps.listSkills) {
        return {
            mode: deps.getMode?.(),
...
function formatCommandCenterSkillList(outcome) {
...
function formatCommandCenterSkillDescription(skill) {
    return skill.whenToUse ? `${skill.description} ${skill.whenToUse}` : skill.description;
}
function formatCommandCenterSkillName(skill) {
