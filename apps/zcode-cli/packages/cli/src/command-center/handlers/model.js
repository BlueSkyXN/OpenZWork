...
export async function handleModelCommand(args, deps, selectedRef) {
...
function resolveTuiModelSelection(args, options, selectedRef) {
    ...
    const option = options.find(({ ref }) => ref.providerId === requested.providerId && ref.modelId === requested.modelId);
    if (!option)
        throw new Error(`Model is not available: ${args}`);
    if (option.disabledReason)
        throw new Error(option.disabledReason);
    ...
    if (!reasoningLevel ||
        ...
        throw new Error(`Select a supported reasoning effort: ${option.reasoning?.levels.map((level) => level.value).join(", ") || "none available"}`);
    }
    return {
