...
export async function handleDwfCommand(args, deps) {
    const app = await deps.getApp();
    const [action = "list", runId] = splitArgs(args);
    if (action === "list") {
        if (!app.listDynamicWorkflowRuns)
    ...
    if (action === "resume") {
        ...
        if (!app.resumeWorkflowRun)
            return unavailable(deps);
        const result = await app.resumeWorkflowRun({ workId: runId });
        return respond(deps, result.ok
...
async function handleCancel(runId, app, deps) {
    if (!app.cancelBackgroundTask)
        return unavailable(deps);
...
function formatRunLine(run) {
    ...
    if (run.resumable)
        columns.push("resumable");
    if (run.updatedAt !== undefined)
        columns.push(`updated ${formatUpdatedAt(run.updatedAt)}`);
...
function formatFailure(run) {
    if (!run.failureCode && !run.failureMessage)
        return "";
    const code = run.failureCode ?? run.status;
    return run.failureMessage ? ` (${code}: ${run.failureMessage})` : ` (${code})`;
}
function describeResumeRejection(reason) {
    switch (reason) {
        case "not_found":
...
function respond(deps, response) {
    return {
        mode: deps.getMode?.(),
        response,
    };
