...
export function createProtocolProcessLifecycle(options = {}) {
    ...
    let drainTimer;
    let completion;
    const stop = (error, code = error ? 1 : 0) => {
        ...
        if (error)
            ...
            drainTimer = setTimeout(() => controller.abort(new Error("Protocol input closed")), PROTOCOL_INPUT_DRAIN_MS);
    };
    const onInputEnd = () => stop();
    const onIoError = (error) => stop(error);
    const onOutputClose = () => stop(new Error("Protocol output closed"));
    const signals = process.platform === "win32"
        ? ["SIGINT", "SIGTERM"]
        : ["SIGINT", "SIGTERM", "SIGHUP"];
    for (const signal of signals) {
        const listener = () => stop(new Error(`Protocol received ${signal}`), SIGNAL_EXIT_CODES[signal]);
