...
export interface ISkillsService {
  ...
  /** 将指定 skill 复制到用户级通用目录（~/.openzwork/skills），成功后返回新 skill 的路径。 */
  copyToCommon(params: {
    workspacePath: string;
    workspaceIdentity?: string;
    skillId: string;
  }): Promise<{ newPath: string }>;
  /** 从通用目录中移除指定 skill（仅当 skill 位于用户级通用目录（~/.openzwork/skills）时有效）。 */
   ...
   * 删除技能所在目录，仅允许命中 .openzwork/skills（用户级）、.zcode/skills（项目级）或 .agents/skills 根，越界则拒绝。
   */
  deleteSkill(params: {
    workspacePath: string;
    workspaceIdentity?: string;
