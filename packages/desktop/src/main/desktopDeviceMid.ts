...
interface EnsureDesktopDeviceMidSyncOptions {
  /** state 文件所在目录，默认 getAppConfigDir()（即 ~/.openzwork/v2）。仅测试注入 */
  configDir?: string;
...
/**
 * 同步确保设备身份文件（磁盘文件名沿用 telemetry-state.json）里有 deviceMid，并返回该值。
 *
 * deviceMid 是设备身份的唯一持久化 UUID，供本地能力（desktop help、preload `--device-id=`
 * 注入）使用；需要在窗口创建前同步取值，故此处用 node:fs 同步读写。
 *
 * 竞态规避：
 * - 已存在合法 deviceMid 时直接返回、绝不写盘（老用户/二次启动零写入）。
 * - 缺失才写，且读出「完整 state」只补 deviceMid 再写回，避免冲掉文件中的其他字段。
