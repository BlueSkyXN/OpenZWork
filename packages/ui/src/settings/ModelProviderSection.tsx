// Modified for the private fork, 2026-09-21: account/OAuth/coding-plan wiring removed —
// this section now manages custom (personal) providers only: CRUD, template creation,
// connectivity test and display order (C1 contract).
...
export function ModelProviderSection({
  ...
  const [initialModelProviderTarget, setInitialModelProviderTarget] = useState(() =>
    consumePendingSettingsModelProviderTarget(),
  );
  const [deferredModelProviderTarget, setDeferredModelProviderTarget] =
    useState<SettingsModelProviderTarget | null>(null);
  ...
  const resolveAndApplyModelProviderTarget = useCallback(
    (target: SettingsModelProviderTarget) => {
      const provider = modelProviders.find(
        (candidate) =>
          candidate.providerId === target.providerId &&
          candidate.config.group === "standard-personal",
      );
      if (!provider) {
  ...
  const applyModelProviderTarget = useCallback(
    (target: SettingsModelProviderTarget | undefined) => {
      if (!target) return;
      if (loading) {
        setDeferredModelProviderTarget(target);
        return;
      }
      resolveAndApplyModelProviderTarget(target);
    },
    [loading, resolveAndApplyModelProviderTarget],
  );

  // Provider 列表仍在加载时暂存目标，加载完成后统一应用。
  useEffect(() => {
    if (loading || !deferredModelProviderTarget) {
      return;
    }
    const target = deferredModelProviderTarget;
    setDeferredModelProviderTarget(null);
    resolveAndApplyModelProviderTarget(target);
  }, [deferredModelProviderTarget, loading, resolveAndApplyModelProviderTarget]);

  useEffect(() => {
    if (!initialModelProviderTarget) {
      return;
    }
    if (loading) {
      return;
    }
    const target = initialModelProviderTarget;
    setInitialModelProviderTarget(undefined);
    resolveAndApplyModelProviderTarget(target);
  }, [initialModelProviderTarget, loading, resolveAndApplyModelProviderTarget]);

  useEffect(() => {
    if (!pendingModelProviderTarget) {
      return;
    }
    applyModelProviderTarget(pendingModelProviderTarget);
    onConsumePendingModelProviderTarget?.();
  ...
  const handleSelectNavItem = useCallback((item: { key: string }) => {
    setInvalidProviderTarget(false);
  ...
  const handleSave = useCallback(
    async (config: Parameters<typeof saveProvider>[0]) => {
      try {
  ...
  const handleDelete = useCallback(
    async (provider: Parameters<typeof confirmAndDeleteModelProvider>[0]["provider"]) => {
